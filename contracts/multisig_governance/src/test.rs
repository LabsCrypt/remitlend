...
#[test]
fn test_reproposal_blocked_during_cooldown() {
    let env = Env::default();
    let client = MultisigGovernanceClient::new(&env, &env.register_contract(None, MultisigGovernance));

    let admin = Address::generate(&env);
    let new_admin = Address::generate(&env);
    setup_governance(&env, &client, &admin);

    // Create and cancel a proposal
    let prop_id = client.propose_admin_transfer(&admin, &new_admin);
    client.cancel_proposal(&admin, &prop_id);

    // Fast-forward timestamp to 10 seconds into a 300-second cooldown
    env.ledger().set_timestamp(env.ledger().timestamp() + 10);

    // Expect error when reproposing within the cooldown period
    let res = client.try_propose_admin_transfer(&admin, &new_admin);
    assert_eq!(res, Err(Ok(Error::CooldownNotElapsed)));
}

#[test]
fn test_reproposal_allowed_after_cooldown() {
    let env = Env::default();
    let client = MultisigGovernanceClient::new(&env, &env.register_contract(None, MultisigGovernance));
    ...
    // Fast-forward timestamp past the 300-second cooldown period
    env.ledger().set_timestamp(env.ledger().timestamp() + 301);

    // Expect success when reproposing after cooldown elapses
    let res = client.try_propose_admin_transfer(&admin, &new_admin);
    assert!(res.is_ok());
}